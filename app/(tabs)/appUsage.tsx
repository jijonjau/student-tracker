...
//   const [isFocused, setIsFocused] = useState(true);
//   const [classSession, setClassSession] = useState(null);
//   const [forceRender, setForceRender] = useState(0); // Force UI updates
//   const appState = useRef(AppState.currentState);
//   const trackingInterval = useRef(null);
//   const checkInterval = useRef(null);
//   const lastUpdateTime = useRef(null); // Track last update for continuity
//   const focusedTimeRef = useRef(0); // Ref to maintain true value
//   const distractedTimeRef = useRef(0); // Ref to maintain true value

//   // Function to update all UI state - called after each time update
//   const updateUIState = useCallback(async () => {
//     setFocusedTime(focusedTimeRef.current);
//     setDistractedTime(distractedTimeRef.current);
//     setForceRender(prev => prev + 1);
...
//     // Increment time based on focus state using refs for reliability
//     if (isFocused) {
//       focusedTimeRef.current += 1;
//       console.log(`Focused time updated: ${focusedTimeRef.current}`);
//     } else {
...
//     lastUpdateTime.current = now.getTime(); // Store last update timestamp
//     await updateUIState();
//   };
...
//     // If no class session, just update state
//     if (!classSession) return;
...
//       setIsFocused(true);
//       console.log('Resuming focused tracking.');
//       // Sync distracted time if app was in background
//       if (lastUpdateTime.current) {
//         const timeAway = Math.floor((now.getTime() - lastUpdateTime.current) / 1000);
//         if (timeAway > 0) {
//           distractedTimeRef.current += timeAway;
...
//       setIsFocused(false);
//       console.log('Switching to distracted tracking.');
//       // Important: Send notification when leaving the app during class
//       await sendDistractionNotification();
//     }
//   };
